...
const GoldenSilhouette: React.FC<{ color: string }> = ({ color }) => (
  <svg viewBox="0 0 120 70" className="golden-silhouette" width="120" height="70" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <ellipse cx="82" cy="22" rx="7" ry="9" fill={color} opacity="0.7" transform="rotate(-15 82 22)" />
    ...
    <path d="M28 40 Q10 25 15 15 Q18 8 22 12 Q20 20 30 30" fill={color} opacity="0.8" className="golden-tail" />
  </svg>
);
...
const GeometricHex: React.FC<{ color: string }> = ({ color }) => (
  <svg viewBox="0 0 60 60" width="60" height="60" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g className="hex-outer">
      <polygon points="30,3 55,17 55,43 30,57 5,43 5,17" stroke={color} strokeWidth="1.5" fill="none" opacity="0.5" />
    </g>
    <g className="hex-inner">
      <polygon points="30,12 48,22 48,38 30,48 12,38 12,22" stroke={color} strokeWidth="1.5" fill="none" opacity="0.7" />
    </g>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-center">
        ...
        <p className="font-serif font-bold text-lg md:text-xl leading-snug max-w-2xl mx-auto" style={{ letterSpacing: '-0.01em', color: textColor }}>
          {fact}
        </p>
      </div>

      {/* Right decorations */}
      <div className="hidden md:flex items-center gap-4 shrink-0" style={{ transform: 'scaleX(-1)' }}>
        <GoldenSilhouette color={silhouetteColor} />
        <GeometricHex color={hexColor} />
      </div>
    </div>
