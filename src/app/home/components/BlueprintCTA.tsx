...
const ServicesMockup: React.FC = () => (
  <div className="relative w-full max-w-xs mx-auto">
    ...
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #E8B800 0%, #C49A00 100%)', transform: 'rotate(6deg)', opacity: 0.6 }}
    />
    <div
      className="absolute top-1 left-2 w-full h-full rounded-2xl border-2 border-honey-gold/40"
      style={{ background: 'linear-gradient(135deg, #C8281E 0%, #A01F16 100%)', transform: 'rotate(3deg)', opacity: 0.5 }}
    />

    {/* Main card */}
    <div
      className="blueprint-mockup relative w-full rounded-2xl overflow-hidden border-2 border-verde/40 shadow-warm-lg"
      style={{ background: 'linear-gradient(135deg, #1B7A3E 0%, #2E9E55 100%)', aspectRatio: '3/4' }}
    >
      <div className="p-8 h-full flex flex-col justify-between">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-honey-gold text-xs font-bold uppercase tracking-widest">Impronta Canina</span>
          </div>
        ...
        <div className="space-y-1.5">
          {['Adiestramiento Individual', 'Asesoramiento Personalizado', 'Cursos Grupales', 'Paseos Profesionales', 'Traslados Seguros'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full" style={{ background: i === 4 ? '#A8E6BE' : i % 2 === 0 ? 'var(--honey-gold)' : 'var(--tangerine)' }} />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6">
              ...
              <p className="text-base leading-relaxed" style={{ color: 'rgba(26,26,26,0.75)' }}>
                En Impronta Canina creemos que cada aprendizaje deja una marca. Trabajamos con método, amor y ciencia del comportamiento para que esa marca sea positiva, duradera y transformadora.
              </p>
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={f.text} className="flex items-center gap-3 text-sm font-medium" style={{ color: '#1A1A1A' }}>
                    <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0" style={{ background: `${f.color}33` }}>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
                ...
                <p className="text-xs font-medium" style={{ color: 'rgba(26,26,26,0.55)' }}>Sin compromiso. Te respondemos en 24 hs.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          {submitted ? (
            <div className="space-y-3">
              ...
              <h3 className="font-serif font-bold text-2xl text-verde">¡Gracias, {name}! Pronto te contactamos.</h3>
              <p className="text-slate-mid text-sm">Revisá tu bandeja de entrada. Te escribimos en menos de 24 horas.</p>
            </div>
          ) : (
            <>
              <div className="inline-block tag-badge px-4 py-1.5 rounded-full mb-4 font-bold text-xs uppercase tracking-wide" style={{ background: 'var(--tangerine)', color: '#ffffff' }}>
                Consulta Gratuita
              </div>
              <h3 className="font-serif font-bold text-2xl md:text-3xl text-charcoal mb-3" style={{ letterSpacing: '-0.01em' }}>
              ...
              <form onSubmit={handleConsulta} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                <input
                  type="text"
                  value={name}
                  ...
                  style={{ background: 'white', color: 'var(--charcoal)', borderColor: 'rgba(232,184,0,0.5)' }}
                  required
                />
                <button
                  type="submit"
              ...
              <p className="text-silver text-xs mt-3">Sin spam. Tu información es confidencial y solo la usamos para contactarte.</p>
            </>
          )}
        </div>
      </div>
